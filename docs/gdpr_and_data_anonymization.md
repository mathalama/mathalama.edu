...
* **Фаза 1 (Синхронная / Мгновенная): Мгновенная блокировка (Grace Period).**
  При запросе на удаление статус пользователя в БД меняется на `deleted_scheduled`, устанавливается отметка `deleted_at = NOW()`, а все его активные сессии инвалидируются в Redis. Вход на платформу блокируется. При этом все файлы в S3 и данные в БД остаются нетронутыми. У пользователя есть 14 дней (период охлаждения), чтобы восстановить аккаунт через службу поддержки.

* **Фаза 2 (Асинхронная / Отложенная): Окончательная анонимизация (Hard Purge).**
  Планировщик (cron-воркер) раз в сутки сканирует БД на наличие пользователей в статусе `deleted_scheduled`, у которых `deleted_at` наступил более 14 дней назад. Для каждого такого пользователя планировщик публикует событие `user.gdpr_delete_requested` в NATS JetStream, запуская процесс физического удаления файлов из S3 и затирания PII-данных в БД.

### 2.1. Юридическое обоснование 14-дневного Grace-периода
Согласно регламенту **GDPR Art. 12(3)** и **Art. 17**, контроллер персональных данных должен предоставить информацию о мерах, принятых по запросу на удаление, *«без неоправданной задержки» (without undue delay)* и в любом случае **в течение одного месяца (30 дней)** с момента получения запроса.
Внедрение 14-дневного льготного периода (Grace Period) полностью укладывается в установленный законом 30-дневный лимит на окончательную обработку. Таким образом, двухфазное удаление является на 100% юридически чистым, защищая при этом платформу от случайных деструктивных действий пользователей и обеспечивая возможность оперативного восстановления.

### 2.2. Процедура восстановления аккаунта (Account Restoration Flow)
Если пользователь обратился в поддержку с просьбой отменить удаление до истечения 14-дневного срока, запускается процесс восстановления:
1.  **Запрос через Панель поддержки:** Администратор или агент поддержки отправляет запрос к API:
    `POST /api/v1/admin/users/{user_id}/restore`
2.  **Транзакция восстановления в БД (Auth Service):**
    В рамках единой транзакции статус пользователя меняется с `deleted_scheduled` обратно на `active`, а поле `deleted_at` сбрасывается в `NULL`:
    ```sql
    UPDATE users 
    SET status = 'active', deleted_at = NULL, updated_at = NOW() 
    WHERE id = :user_id AND status = 'deleted_scheduled';
    ```
3.  **Публикация события отмены удаления:**
    Бэкенд генерирует в outbox-таблицу событие `user.gdpr_restore_requested`, которое публикуется в NATS JetStream.
4.  **Обработка события ведомыми сервисами:**
    *   **Notification Service** отправляет студенту email-уведомление о восстановлении доступа.
    *   **Progress Service** и другие ведомые компоненты сбрасывают отложенные таймеры удаления и возвращают пользователя в активные потоки.
5.  **Результат:** Доступ полностью восстановлен, все данные и прогресс сохранены. Если же 14-дневный лимит превышен — cron-воркер уже выполнил анонимизацию, и восстановление невозможно ни технически, ни юридически.

### Схема жизненного цикла процесса удаления и восстановления:

```mermaid
sequenceDiagram
    autonumber
    actor Student as Студент
    actor Admin as Администратор поддержки
    ...
    alt Сценарий А: Студент просит отменить удаление на 5-й день
        Admin->>API: POST /api/v1/admin/users/{id}/restore
        API->>DB: UPDATE users SET status = 'active', deleted_at = NULL
        API->>NATS: Публикация события user.gdpr_restore_requested
        API-->>Admin: HTTP 200 OK ("Аккаунт успешно восстановлен")
    else Сценарий Б: Проходит 14 дней без запроса восстановления
    ...
    Note over Worker: Асинхронная очистка PII (Сценарий Б)
