...
*   **Request Fields:**
    ...
    *   `student_notes` (Text, необязательный комментарий студента)
...
*   **Обязательные правила валидации на бэкенде:**
    1. Поле `question_text` должно быть заполнено и не быть пустым.
    2. Поле `question_type` должно иметь значение строго `single`, `multiple` или `text`.
    3. Для вопросов типа `single` и `multiple` массив вариантов `options` должен содержать не менее 2 вариантов ответа.
    4. Для вопросов типа `single` (одиночный выбор) должен быть выбран **ровно один** правильный ответ (`is_correct = true`).
    5. Для вопросов типа `multiple` (множественный выбор) должен быть выбран **хотя бы один** правильный ответ (`is_correct = true`).
    6. Для вопросов типа `text` (свободный текстовый ввод) массив `options` должен быть пустым (ответ проверяется куратором или точным соответствием регулярному выражению).
*   **Request Body:**
    ```json
    [
      {
...
*   **Response (200 OK):**
    ...
    ```
*   **Response (400 Bad Request при ошибке валидации):**
    ```json
    {
...
*   **Request Body:**
    ```json
    {
      "title": "Математика: Основы сложения и вычитания",
      "description": "Базовый курс по арифметике для развития логического мышления."
    }
    ```
*   **Response (201 Created):**
    ```json
