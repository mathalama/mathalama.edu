...
3. **Ограничение времени блокировки (Lock Timeout):**
   ...
   SET lock_timeout = '4s';
   ```
4. **Обязательное разделение на два файла миграции (.up и .down):**
   Каждая миграция в репозитории обязана состоять строго из двух связанных SQL-файлов с одинаковым префиксом версии:
   * **`.up.sql`** — содержит запросы на развертывание (расширение) схемы, создание таблиц, добавление полей, индексов и типов.
   * **`.down.sql`** — содержит зеркальные запросы на откат изменений (сужение схемы), возвращающие состояние БД к предыдущей версии.
   * *Критическое требование:* Любой `.up.sql` файл обязан иметь соответствующий `.down.sql` файл. Миграции без файла отката отклоняются на этапе CI/CD автоматическими линтерами.
5. **Инструментарий миграций:**
