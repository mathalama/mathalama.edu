...
#### Резервный сценарий (Fallback) при падении `Auth Service` с асимметричной верификацией:
Если `Auth Service` временно недоступен и Circuit Breaker переходит в состояние `OPEN`, для предотвращения брешей в безопасности верификация токенов в `Progress Service` переводится на следующий защищенный сценарий:
*   **Асимметричное шифрование (RS256 / EdDSA):** В штатном режиме `Auth Service` подписывает JWT-токены с помощью приватного ключа. Все остальные сервисы содержат в памяти закэшированный публичный ключ (полученный при старте или через локальную JWKS-директорию), что исключает необходимость синхронного похода в `Auth Service` за валидацией подписи. Даже компрометация одного из ведомых микросервисов не позволит злоумышленнику выпустить валидный JWT, так как приватный ключ хранится исключительно в защищенном контуре `Auth Service` (интегрированном с Vault).
*   **Временное окно уязвимости (Short TTL):** Время жизни JWT жестко ограничено 15 минутами.
*   **Синхронизация черного списка (Local Revocation Cache / Bloom Filter):** Для мгновенного отзыва токенов при выходе кураторов или блокировке студентов, `Auth Service` публикует события отзыва токенов в NATS, а ведомые сервисы накапливают их в локальной In-Memory таблице (или Bloom-фильтре) с TTL 15 минут.
*   **Строгий запрет мутаций (Read-Only Fallback):** В fallback-режиме (при недоступности `Auth Service` по gRPC) ведомым сервисам **строго запрещено выполнять любые пишущие транзакции** (POST, PUT, DELETE, включая загрузку PDF-конспектов или отправку ответов на тесты). Студент может беспрепятственно продолжать читать текстовую теорию и смотреть видеоуроки (так как контент доступен по проверенной локально подписи JWT), но не может изменить состояние системы. Пользователь видит предупреждение: *"Связь с сервером авторизации временно ограничена. Доступен только просмотр материалов"*.
...
### 2.5. Опция Модульного Монолита (Phase 1 Deployment)

Хотя MathalamaEdu спроектирована как распределенная микросервисная система для масштабирования, на начальном этапе (при обслуживании сотен или тысяч студентов) развертывание 5 отдельных сервисов, mTLS, Vault и распределенного трейсинга создает избыточную операционную и инфраструктурную сложность (Over-engineering).

Для снижения TTM (Time to Market) и упрощения отладки в кодовой базе закладывается **поддержка сборки в виде Модульного Монолита**:
1.  **Логическая изоляция в Go:** Каждый микросервис разрабатывается как изолированный модуль (вложенный Go package) со своими Use Case и Repository интерфейсами. Взаимодействие между модулями инкапсулируется внутри интерфейсов-медиаторов.
2.  **Компиляция в единый бинарник:** На этапе CI/CD с помощью флагов сборки (Build Tags) все модули могут быть скомпилированы в один исполняемый файл монолита.
3.  **Единая PostgreSQL СУБД:** Вместо 5 баз данных используется один инстанс PostgreSQL, но таблицы модулей разделяются по логическим схемам (`auth.*`, `course.*`, `progress.*`) для предотвращения сквозных JOIN-запросов на уровне SQL. Связи между схемами остаются чисто концептуальными, внешние ключи не пересекают границы модулей.
4.  **In-Process обмен сообщениями:** Для асинхронных коммуникаций на Phase 1 вместо внешнего кластера NATS JetStream может использоваться локальный in-memory брокер (например, Go-channels или локальный embedded-NATS сервер), запускаемый в рамках того же процесса монолита.
5.  **Плавный переезд (Seamless Migration):** Когда нагрузка вырастет, модули монолита физически разносятся по отдельным репозиториям и Docker-контейнерам без переписывания бизнес-логики — достаточно заменить in-process вызовы медиаторов на gRPC-клиенты и подключить полноценный брокер NATS JetStream.
...
#### 1. Спецификация и гарантии надежности кэша (Durability Trade-off):
*   **Окно потери данных:** Задержка в 10 секунд между записью в Redis и сбросом в PostgreSQL — это осознанный компромисс ради производительности. При падении кэша данные за последние 10 секунд могут быть потеряны. Для видеопрогресса это некритично (студент просто начнет просмотр на несколько секунд раньше).
*   **Снижение рисков (Redis AOF):** Для минимизации катастрофических потерь инстанс Redis настраивается с включенным механизмом **AOF (Append-Only File)** с политикой `appendfsync everysec`. Это сокращает фактическое окно потери данных до **1 секунды** при аппаратном сбое ноды Redis.
*   **Обход кэша (PostgreSQL Bypass) для критических действий:** Любые действия, переводящие урок в финальный статус «пройдено» (сдача теста или апрув домашней работы куратором), **полностью обходят** асинхронный Write-Behind кэш. Они пишутся напрямую в PostgreSQL с полной транзакционной гарантией ACID.

#### 2. Схема записи (In-Memory HSET/SADD)
Когда от студента поступает запрос о промежуточном просмотре видеолекции:
...
#### 3. Фоновый сброс (Batch Ingestion via CacheFlusher)
...
### 7.3. Обеспечение консистентности: Монотонное слияние (Monotonic Rank Merge)
Для предотвращения рассинхронизации данных при обновлении личного кабинета (когда прогресс в Redis еще не сброшен на диск) чтение выполняется по гибридному сценарию с использованием монотонных рангов статусов и временных меток:
1.  **Монотонный ранг состояний:** Каждому статусу прогресса присваивается строгий весовой коэффициент:
    *   `LOCKED = 0`, `UNLOCKED = 1`, `COMPLETED = 2`.
2.  **Запрос данных:** Use Case считывает базовый прогресс из PostgreSQL и свежий кэш из Redis (`HGETALL progress:pending:{student_id}`).
3.  **Логика слияния (Merge):** Вместо хрупкого строкового сравнения, статус урока вычисляется как функция максимума от весов статусов:
    $$\text{FinalStatus} = \text{MapToStatus}(\max(\text{Weight}(\text{Status}_{\text{DB}}), \text{Weight}(\text{Status}_{\text{Redis}})))$$
    Если веса равны, приоритет отдается записи с более поздней временной меткой `updated_at`. Это полностью исключает логические сбои при изменении или добавлении новых шагов обучения.

### 7.4. Безопасность и валидация загрузки ДЗ: Многоуровневый S3 Webhook Pipeline
Для исключения битых ссылок (когда студент запросил Presigned URL, но не загрузил файл) и загрузки вредоносного ПО/эксплоитов под видом PDF:
...
3.  **Многоуровневая верификация асинхронным воркером `S3VerificationWorker`:**
    *   **Шаг 1 (Сигнатурный анализ Magic Bytes):** Считывание первых 4 байт файла непосредственно из хранилища для проверки реального MIME-типа (сигнатура `%PDF` - `0x25 0x50 0x44 0x46`).
    *   **Шаг 2 (Структурный синтаксический анализ):** Воркер пытается частично распарсить PDF-файл с помощью специализированной библиотеки (`pdfcpu` или `rsc.io/pdf`) для проверки целостности структуры документа. Это полностью защищает от "polyglot" файлов (сшитых исполняемых файлов с `%PDF` в заголовке).
    *   **Шаг 3 (Антивирусное сканирование):** Воркер передает поток файла на проверку во встроенный антивирусный сервис-сайдкар (**ClamAV Daemon**).
    *   **Шаг 4 (Проверка лимита размера):** Проверяется соответствие жесткому лимиту в 20 МБ.
    *   **Результат:** Только при успешном прохождении всех шагов воркер переводит статус субмита в `pending` (ожидает проверки куратором). Иначе статус сбрасывается в `rejected` с детальным логом ошибки.
4.  **Безопасность раздачи (S3 Headers):** При отдаче файлов кураторам S3 настраивается на строгое принудительное выставление заголовков `Content-Type: application/pdf` и `Content-Disposition: attachment; filename="homework.pdf"`. Это полностью предотвращает выполнение любых встроенных HTML/JS скриптов в контексте домена приложения.

### 7.5. Надежность Telegram Bot FSM
Для предотвращения потери состояний кураторов (например, в процессе ввода оценки) при падении или перезапуске кэша:
1.  **Персистентный Redis:** Инстанс Redis, обслуживающий сессии Telegram-бота, настраивается с периодическими снимками (RDB) и постоянной дозаписью логов (AOF).
2.  **SQL-черновики (Review Drafts):** Любые промежуточные значимые бизнес-действия куратора (например, ввод развернутого текстового фидбека для студента) перед отправкой и коммитом оценки дублируются в таблицу `review_drafts` в PostgreSQL. При сбое сессии бот автоматически восстанавливает контекст куратора из СУБД.
3.  **Stateless Callback Data:** Вся навигационная логика бот-меню кодируется непосредственно в inline-кнопках Telegram (`callback_data`), что делает меню полностью независимым от состояния сервера.

### 7.6. Защита от Race Conditions при конкурентной обработке в NATS JetStream
При горизонтальном масштабировании воркеров в Queue Group сообщения распределяются по инстансам round-robin. Если для одного student_id отправляются несколько сообщений подряд, они могут обрабатываться воркерами параллельно, создавая риск состояния гонки (Race Condition). Для гарантии порядка и консистентности:
1.  **Subject-Based Partitioning:** Если требуется обработка строго на одном инстансе, тема разбивается на фиксированные партиции (например, `...submission.approved.part-N`), где издатель хэширует `student_id` для выбора темы, а воркеры слушают конкретные партиции.
2.  **Оптимистичная блокировка (OCC):** В СУБД используется контроль версий сущностей (столбец `version` увеличивается при каждом изменении, транзакция падает при несовпадении версий).
3.  **Распределенные блокировки в Redis:** Перед началом обработки события по студенту воркер пытается захватить блокировку `lock:student:{id}` на фиксированный TTL. Если блокировка занята, событие возвращается в NATS (Nack со временем задержки).
