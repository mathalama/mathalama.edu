...
Платформа **MathalamaEdu** использует RESTful API для взаимодействия между фронтендом и бэкендом. Все запросы и ответы осуществляются в формате **JSON**. Авторизация выполняется с помощью **JWT-токенов** (асимметричная подпись RS256/EdDSA), передаваемых в заголовке `Authorization: Bearer <token>`.

---

## Лимиты запросов (API Rate Limiting Policy)

Для обеспечения высокой доступности и защиты от сетевых атак в платформе действует двухкомпонентная система ограничения частоты запросов (Rate Limiting):

1.  **Глобальный лимит (Token Bucket на уровне Nginx API Gateway):**
    *   Все стандартные эндпоинты платформы ограничены лимитом **100 запросов в минуту** на один IP-адрес. При превышении возвращается HTTP статус `429 Too Many Requests`.
2.  **Строгие точечные лимиты (Sliding Window Rate Limiter через Redis):**
    *   **POST `/api/v1/auth/login` (Защита от перебора паролей / Brute-Force):** Строго ограничен лимитом **5 запросов в минуту** на один IP-адрес.
    *   **POST `/api/v1/student/lessons/{id}/submit` (Предотвращение спама в S3 и перегрузки антивирусного конвейера):** Ограничен лимитом **3 запроса в минуту** на одного авторизованного Студента (`student_id`).

При срабатывании лимитов возвращается стандартизированный ответ:
```json
{
...
*   **Доступ:** Публичный
*   **Ограничение частоты (Rate Limit):** Strict Sliding Window в Redis — **5 запросов в минуту** на один IP.
*   **Request Body:**
    ```json
    {
...
*   **Описание:** Студент инициирует загрузку написанного от руки конспекта или ДЗ. Бэкенд создает запись субмита со статусом `upload_pending`, генерирует безопасный **S3 Presigned URL** для прямой загрузки файла в MinIO и возвращает его клиенту. Студент загружает файл напрямую в MinIO S3. После завершения загрузки MinIO вызывает webhook-событие в NATS, запускающее многоуровневую верификацию в воркере `S3VerificationWorker`.
*   **Content-Type:** `application/json`
*   **Ограничение частоты (Rate Limit):** Sliding Window в Redis — **3 запроса в минуту** на одного `student_id`.
*   **Правила валидации и лимиты безопасности:**
    1.  **Сигнатурный анализ (Magic Bytes):** Воркер считывает первые 4 байта файла непосредственно из S3 (допускается только валидная PDF сигнатура `%PDF` / `0x25 0x50 0x44 0x46`).
    2.  **Структурная валидация:** Бэкенд пытается частично распарсить PDF с помощью AST-парсера (`pdfcpu` или `rsc.io/pdf`) для проверки структурной целостности документа. Это предотвращает загрузку вредоносных "polyglot" файлов (сшитых с исполняемым кодом).
    3.  **Антивирусный анализ:** Поток файла передается на проверку во встроенный сайдкар-антивирус **ClamAV Daemon**.
    4.  **Размер файла:** Строгий лимит максимум **20 МБ**.
    5.  **Ограничение спама в S3:** Не более **3 активных субмитов** (в статусах `upload_pending`, `pending` или `rejected`) на этот урок от одного студента.
    6.  **Длина комментария `student_notes`:** Не более **5 000 символов**.
    7.  **Заголовки раздачи (Safety Headers):** При раздаче PDF-документов кураторам система жестко принудительно выставляет заголовки безопасности `Content-Type: application/pdf` and `Content-Disposition: attachment; filename="homework.pdf"` для полной блокировки запуска встроенных скриптов в контексте домена.
*   **Request Body:**
    ```json
    {
...
*   **Описание:** Отмечает видеолекцию как просмотренную студентом. Для обеспечения высокой пропускной способности при пиковых нагрузках (Write-Heavy) используется паттерн **Progress Write-Behind Cache** с использованием Redis In-Memory. Запись в PostgreSQL отложена на 10 секунд и выполняется фоновым демоном.
*   **Производительность:** Мгновенный ответ за **<1мс** без блокировок СУБД.
*   **Headers:** `Authorization: Bearer <token>`
*   **Response (200 OK):**
    ```json
    {
      "message": "Прогресс просмотра видеолекции успешно сохранен в кэше памяти.",
      "cached_at": "2026-05-22T22:00:15Z"
    }
    ```
